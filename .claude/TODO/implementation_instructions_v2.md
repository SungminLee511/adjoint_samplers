...
1. **`neural_stein_cv.py`** — Independent of RKHS path. Needs autodiff. Test on Gaussians first.
...
**Stein CV working:** Variance reduction factor < 0.5 (50%+ reduction). **Critically, also check error reduction** — by the bias-variance coupling theorem (MATH_SPEC Section 2.7), lower variance implies lower bias. If the Stein CV estimate has lower error than the naive estimate across seeds, this confirms that variance reduction is simultaneously correcting bias. This is the most important result to look for — it validates that Stein CVs can partially correct the distributional mismatch $q_\theta \neq p$ without any MCMC correction. Effect should be stronger on DW4 (12D) than LJ55 (165D) due to kernel scaling.

**Stein CV bias reduction specifically:** Compare `error_stein` vs `error_naive` in the summary. If `error_stein < error_naive` consistently across seeds, the Stein CV is reducing bias. The theoretical bound is $|\text{Bias}| \leq \sqrt{C \cdot \text{Var}*{q*\theta}[h_g]}$, so a 4× variance reduction should give up to 2× bias reduction (square root relationship).
...
## Task 11: Neural Stein CV via Differentiated Poisson Equation (`enhancements/neural_stein_cv.py`)

This is the most advanced enhancement. See MATH_SPEC Section 7 for the full derivation. The key insight: differentiating the Poisson equation $f + \mathcal{A}_p g = c$ eliminates the unknown constant $c$, yielding a PDE for $g$ that can be solved by a neural network.

### 11.1 Functions to Implement
...
def neural_stein_cv_loss(
    ...
    Args:
        ...
        f_grad: (B, d) precomputed grad_f(x). If None, computed via autodiff
            from f_values (requires f_values to have grad_fn).
    ...
    # grad_x h — the quantity that should be zero
...
def train_neural_stein_cv(
    ...
    with torch.no_grad():
        all_scores = energy.score(samples)  # (N, d)
    ...
    g_model.eval()
    with torch.no_grad():
        # We need grad for A_p g computation
        pass
    ...
    for start in range(0, N, batch_size):
        ...
        Apg_b = compute_stein_operator(
            g_b, x_b, s_b, g_model, hutchinson_samples=0,  # exact for final eval
        )
        h_b = f_b + Apg_b
...
```python
def test_neural_stein_cv():
    """Test on a simple Gaussian: p = N(0, I_d), f(x) = ||x||^2.

    Ground truth: E_p[||X||^2] = d (sum of d independent chi-squared(1) variables).
    """
    d = 10
    N = 2000

    # "Target" is N(0, I)
    # "Biased samples" from N(0.5, I) — shifted
    samples = torch.randn(N, d) + 0.5
    scores = -samples  # Score of N(0,I) evaluated at biased locations: s(x) = -x

    class SimpleEnergy:
        def score(self, x): return -x
        def eval(self, x): return 0.5 * (x ** 2).sum(dim=-1)

    energy = SimpleEnergy()
    f_func = lambda x: (x ** 2).sum(dim=-1)  # f(x) = ||x||^2

    g_model = NeuralSteinCV(dim=d, hidden_dim=128, n_layers=3)

    result = train_neural_stein_cv(
        g_model, samples, energy, f_func,
        n_epochs=1000, batch_size=256, lr=1e-3,
    )

    gt = float(d)  # E[||X||^2] = d for X ~ N(0,I)
    print(f"Ground truth:    {gt:.4f}")
    print(f"Naive estimate:  {result['naive_estimate']:.4f} (biased, should be ~{d + d*0.25:.2f})")
    print(f"Neural CV est:   {result['estimate']:.4f} (should be closer to {gt})")
    print(f"Var reduction:   {result['variance_reduction']:.4f}")
```

### 11.3 Integration with Enhanced Evaluator

Add to `enhanced_evaluator.py` after the existing methods:

```python
# In evaluate_enhanced(), after the Generator Stein CV section:

# --- 8. Neural Stein CV ---
from enhancements.neural_stein_cv import NeuralSteinCV, train_neural_stein_cv

g_model = NeuralSteinCV(dim=D, hidden_dim=min(256, max(64, D * 2)), n_layers=3)
...
neural_result = train_neural_stein_cv(
    ...
    samples,
    energy,
    f_func=lambda x: energy.eval(x),  # Estimate mean energy
    n_epochs=500,
    batch_size=min(256, N),
    lr=1e-3,
    hutchinson_samples=1,  # Use 0 for exact (slower) in low-d
    verbose=False,
)
...
if gt_mean_energy is not None:
    results['error_neural_cv'] = abs(neural_result['estimate'] - gt_mean_energy)
```

### 11.4 Key Implementation Notes

1. **Activation functions**: Must be twice differentiable. SiLU (Swish) is recommended — it’s smooth, has good gradients, and is the same activation used in the EGNN model in the ASBS codebase.
1. **Hutchinson vs. exact divergence**: For DW4 (12D), exact divergence (d=12 backward passes) is cheap. For LJ55 (165D), use Hutchinson with 1–5 probes. The Hutchinson estimator adds noise to the loss but the gradient is unbiased.
1. **Gradient clipping**: Essential. The PDE loss involves second derivatives of $g_\phi$, which can explode early in training. Clip at `max_norm=10.0`.
1. **Learning rate schedule**: Cosine annealing helps convergence. Start at 1e-3, decay to ~1e-5.
1. **Number of epochs**: 500 is usually sufficient for DW4. LJ13 may need 1000. LJ55 may need 2000. Monitor the PDE loss — when it plateaus, stop.
