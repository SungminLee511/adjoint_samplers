#!/usr/bin/env python
"""LJ13 Evaluation — Phases 2-5 combined.

All 9 SML methods with practical hyperparameters for LJ13 (39D).

Usage:
    /root/miniconda3/envs/Sampling_env/bin/python -u experiments/run_lj13_eval.py
"""
import sys, os
...
def main():
    ...
    # NFE=1000 for LJ13
    timesteps = torch.linspace(0, 1, 1000, device=device)
    ...
    # Eval config — adapted for LJ13 (39D)
    eval_config = EvalConfig(
        n_seeds=10,
        sample_sizes=[100, 500, 1000, 2000],
        max_stein_samples=2000,
        eval_batch_size=1000,  # smaller batch for LJ13 (more expensive fwd)
        # Neural CV (MLP)
        neural_cv_epochs=500,
        neural_cv_hidden_dim=128,
        neural_cv_n_layers=3,
        neural_cv_batch_size=256,
        neural_cv_lr=1e-3,
        # EGNN CV
        egnn_cv_epochs=500,
        egnn_cv_hidden_nf=64,
        egnn_cv_n_layers=4,
        egnn_cv_batch_size=256,
        egnn_cv_lr=1e-3,
        # RBF
        rbf_n_centers=200,
        rbf_reg_lambda=1e-6,
